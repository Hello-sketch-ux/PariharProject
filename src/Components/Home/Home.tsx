      ...
      >
        ...
        <div className="hidden md:flex space-x-6">
          {["HOME", "SHOP", "ABOUT", "CONTACT", "RESTROOM FINDER"].map((item, index) => (
            <Link key={index} to={`/${item.toLowerCase().replace(/\s/g, "-")}`} className="text-white text-sm font-semibold hover:text-gray-300 transition">
              {item}
            </Link>
          ))}
        </div>
        ...
        {mobileMenuOpen && (
          <div className="absolute top-16 left-0 w-full bg-black bg-opacity-80 flex flex-col items-center space-y-4 py-4 md:hidden">
            {["HOME", "SHOP", "ABOUT", "CONTACT", "RESTROOM FINDER"].map((item, index) => (
              <Link key={index} to={`/${item.toLowerCase().replace(/\s/g, "-")}`} className="text-white text-sm font-semibold">
                {item}
              </Link>
            ))}
        ...
        {isLoggedIn ? (
          <button onClick={() => navigate("/profile")} className="hidden md:block text-white hover:bg-white hover:text-black p-2 rounded-full transition">
            <User size={24} />
          </button>
        ) : (
          <button onClick={() => navigate("/login")} className="hidden md:block border-2 text-white hover:bg-white hover:text-black px-4 py-2 rounded-lg font-semibold transition">
            LOGIN
          </button>
        )}
      </nav>
